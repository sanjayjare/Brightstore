...
foreach($db->query("select row_id,userName, password, statusUrl, companyID, companyName, statusVersion, shippingUrl, shippingVersion from Companies where userName != '' and  password != '' and statusUrl != '' and statusVersion != '' and row_id='".$Supplier."'  order by companyName") as $rows) {
	...
	if($Supplier != null){
		...
		$shippingUrl = isset($suppliers[$Supplier]['shippingUrl']) ? $suppliers[$Supplier]['shippingUrl'] :"";
		$shippingVersion = isset($suppliers[$Supplier]['shippingVersion']) ? $suppliers[$Supplier]['shippingVersion'] : "";
	}
...
print_r('</pre>');*/
/*$sherrormsgdiv="";
$sherror=isset($resultSh['errorMessage']) ? $resultSh['errorMessage'] : "";
if($sherror != ""){
	$sherrorcode=isset($resultSh['errorMessage']['code']) ? $resultSh['errorMessage']['code'] : "";
	$sherrormsg=isset($resultSh['errorMessage']['description']) ? $resultSh['errorMessage']['description'] : "";
	if($sherrorcode != "0"){
		$sherrormsgdiv = "<div style='color:red;'><strong>Order Shipment Notification Error Message</strong><br />Code: $sherrorcode<br /> Description: $sherrormsg</div>";
	}
}
echo $sherrormsgdiv."<br />";*/
...
//$orcount = count($orstatus);

/*print_r('<pre>');
print_r($resultr['OrderStatusArray']['OrderStatus']);
print_r('</pre>');*/
?>
<div class="gridline">
